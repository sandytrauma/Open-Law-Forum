...
const section_ida: React.FC = async () => {
  // Change the fileName as needed
  const fileName = 'IDA.json';
  const data: ActSections = await fetchActSections(fileName);

  return (
    <div>
      <h1 className="text-center mt-5 text-teal-800 text-2xl md:text-3xl font-mono font-extrabold">The Divorce Act, 1869</h1>
      <ActSectionContent data={data} />
    </div>
  );
};

export default section_ida;